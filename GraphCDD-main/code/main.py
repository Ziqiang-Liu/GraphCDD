...
def CDA(n_fold):
    ...
    with open('./results/circ4-dis-drug-3loss-2yuan.txt', 'a') as f:
        f.write('time:\t'+ str(localtime)+"\n")


        
        for train_index, test_index in kf.split(cd_pairs):
            c_drugmatix,train_cd_pairs,test_cd_pairs = load_data.C_Dmatix(cd_pairs,train_index,test_index)
            dataset['c_d']=c_drugmatix
         
            cir_fea, drugfea,dis_fea = load_data.feature_representation(model, args, dataset)
            print(cir_fea.shape)
            print(drugfea.shape)
            print(dis_fea.shape)
            train_dataset = load_data.new_dataset(cir_fea, drugfea,train_cd_pairs)
            test_dataset = load_data.new_dataset(cir_fea, drugfea,test_cd_pairs)
